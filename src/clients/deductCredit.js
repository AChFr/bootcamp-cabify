...
export default async (messageCost) => {
    ...
    try {
        ...
        const existingCreditOnMain = await mainDatabase.findOneAndUpdate({}, { $inc: { amount: -messageCost } },
            { new: true })

        try {

            await secondaryDatabase.findOneAndReplace({}, { amount: existingCreditOnMain.amount },
